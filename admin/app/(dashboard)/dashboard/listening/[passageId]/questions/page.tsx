      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-2">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-gray-900 dark:text-white mb-1 flex items-center gap-2">
            <span className="inline-block bg-gradient-to-r from-green-500 to-blue-600 text-transparent bg-clip-text">Questions</span>
            <span className="ml-2 inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700 dark:bg-green-900/30 dark:text-green-200">{passage.questions?.length || 0}</span>
          </h1>
          <p className="text-gray-500 dark:text-gray-300 text-sm">Manage questions for: <span className="font-semibold">{passage.title}</span></p>
        </div>
        <Button asChild className="h-11 px-6 text-base font-semibold bg-gradient-to-r from-green-600 to-blue-700 hover:from-green-700 hover:to-blue-800 shadow-md">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50 dark:bg-gray-800">
                <TableHead className="text-gray-700 dark:text-gray-200 font-semibold">Question</TableHead>
                <TableHead className="text-gray-700 dark:text-gray-200 font-semibold">Type</TableHead>
                <TableHead className="text-gray-700 dark:text-gray-200 font-semibold">Order</TableHead>
              ...
              {!passage.questions || passage.questions.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-10 text-gray-400 dark:text-gray-500 text-lg">
                    No questions found. Click <span className="font-semibold text-green-600 dark:text-green-400">Add Question</span> to add your first one!
                  </TableCell>
                </TableRow>
              ) : (
                passage.questions.map((q: any) => (
                  <TableRow key={q._id} className="hover:bg-green-50/60 dark:hover:bg-green-900/30 transition-colors">
