...
export default function PassageEditPage({ params }: { params: { passageId: string } } | { params: Promise<{ passageId: string }> }) {
  ...
  const onSubmit = async (data: PassageFormValues) => {
    ...
    if (res.ok) {
      router.push(`/dashboard/reading/${passageId}/view`);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-xl bg-white dark:bg-gray-900 rounded-xl shadow-lg p-8 border border-gray-100 dark:border-gray-800 mx-auto mt-8">
      <h2 className="text-2xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-indigo-600">Edit Passage</h2>
      ...
      {isSubmitSuccessful && <div className="text-green-600 text-sm font-medium bg-green-50 border border-green-200 rounded p-2">Passage updated successfully!</div>}
